    ...
    <ion-content :fullscreen="true">
      <div style="margin-top: 15px">
        <ion-button @click="goBack()">
          Go Back<br />ionRouter.back()<br />custom animation:
      ...
      <div style="margin-top: 15px">
        <ion-button v-if="isPreviousPage" @click="goToPreviousDetailPage">
          Go To Detail Page {{ _previousPageNumber
          }}<br />ionRouter.navigate<br />custom animation: animationBuilder1
          with replace
        </ion-button>

        <ion-button v-else disabled class=".ion-text-capitalize">
          Go To Detail Page {{ _previousPageNumber }}<br />ionRouter.navigate
          with replace<br />custom animation: animationBuilder1
        </ion-button>
        <ion-button @click="goToNextDetailPage()">
          Go To Detail Page {{ _nextPageNumber }}<br />ionRouter.navigate with
          replace<br />custom animation: animationBuilder1
        </ion-button>
      </div>
...
const goToPreviousDetailPage = () => {
  ...
  ionRouter.navigate(
    `/detail/${_previousPageNumber.value}`,
    "back",
    "replace",
    animationBuilder1
  );
